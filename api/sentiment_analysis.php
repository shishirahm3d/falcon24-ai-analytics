...
if (!$input || !isset($input['text'])) {
    ...
    echo json_encode(['success' => false, 'error' => 'Invalid input - text not provided']);
    exit;
}

// 'data' for chat.php will be 'text' from this request
$input['data'] = $input['text'];
unset($input['text']); // Remove original key if it conflicts

// Add the specific task to the input data
$input['task'] = 'sentiment_analysis';
